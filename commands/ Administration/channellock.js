...
module.exports = {
  ...
  aliases: ["chlock", "lockchannel", "lock"],
  cooldown: 2,
  usage: "channellock",
    ...
    try {
      ...
      if(channel.isThread())
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:icons_Wrong:988298434354249809> **This Channel is a Thread u can't Lock it!**`)
        ]});
      if(channel.permissionOverwrites.cache.size < 1){
        ...
        if(channel.permissionOverwrites.cache.filter(permission => permission.allow.toArray().includes("SEND_MESSAGES")).size < 1)
          return message.reply({embeds :[new MessageEmbed()
            .setColor(es.wrongcolor)
            .setFooter(client.getFooter(es))
            .setTitle(`<:icons_Wrong:988298434354249809> **This Channel is locked!**`)
          ]});
        await channel.permissionOverwrites.set(
      ...
      message.reply({embeds :[new MessageEmbed()
        .setColor(es.color)
        .setFooter(client.getFooter(es))
        .setTitle(`<:icons_Correct:987931568939102228> **Successfully locked \`${channel.name}\`**`)
      ]});
