...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      if(member){
        ...
        try{
          member.send({embeds : [new MessageEmbed()
            ...
            .setFooter(client.getFooter(es))
            .setAuthor(`Message from: ${message.author.username}`, message.author.displayAvatarURL({dynamic:true}), "https://discord.gg/DcR5n6PTFf")
        ...
        var members = message.guild.members.cache.filter(member=> member.roles.cache.has(role.id) && !member.user.bot).map(this_Code_is_by_RaffiDev_8321 => this_Code_is_by_RaffiDev_8321);  
        var failed = [];
        var succeeded = [];
        message.delete().catch(e => console.log("Couldn't delete msg, this is a catch to prevent crash"))
        ...
        await message.reply({embeds :[new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
          .setFooter(client.getFooter(es))
          .setAuthor(`Dming ${members.length} Members...`, "https://images-ext-1.discordapp.net/external/ANU162U1fDdmQhim_BcbQ3lf4dLaIQl7p0HcqzD5wJA/https/cdn.discordapp.com/emojis/756773010123522058.gif", "https://discord.gg/stromz")
        ...
        for(const member of members) {
          try{
            ...
            await member.send({embeds : [new MessageEmbed()
              .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
              .setFooter(client.getFooter(es))
              .setAuthor(`Message from: ${message.author.username}`, message.author.displayAvatarURL({dynamic:true}), "https://discord.gg/stromz")
              .setDescription(args.slice(1).join(" ").substr(0, 2048))
            ]}).catch(e=>{
              failedd = true
            })
      ...
      if(client.settings.get(message.guild.id, `adminlog`) != "yes"){
        try{
          var channel = message.guild.channels.cache.get(client.settings.get(message.guild.id, `adminlog`))
          if(!channel) return client.settings.set(message.guild.id, "yes", `adminlog`);
 ...
 * @INFO
 * Work for Stromz Development | https://dash.stromz.ml
 * @INFO
