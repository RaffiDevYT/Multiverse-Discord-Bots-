...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      embed.addField("**<:arrow:988302456452681758> Rules Channel:**", `${message.guild.rulesChannel ? `<#${message.guild.rulesChannelId}>`: "<:icons_Wrong:988298434354249809> \`No Channel\`"}`, true)
      embed.addField("**<:arrow:988302456452681758> Public Updates Channel:**", `${message.guild.publicUpdatesChannel ? `<#${message.guild.publicUpdatesChannelId}>`: "<:icons_Wrong:988298434354249809> \`No Channel\`"}`, true)
      embed.addField("**<:arrow:988302456452681758> AFK Channel:**", `${message.guild.afkChannel ? `<#${message.guild.afkChannelId}>`: "<:icons_Wrong:988298434354249809> \`No Channel\`"}`, true)

      embed.addField("**<:arrow:988302456452681758> NSFW Level:**", `\`${message.guild.nsfwLevel}\``, true)
      embed.addField("**<:arrow:988302456452681758> Verifcation Level:**", `\`${message.guild.verificationLevel}\``, true)
      embed.addField("**<:arrow:988302456452681758> Explicit Content Filter:**", `\`${message.guild.explicitContentFilter}\``, true)
      ...
      embed.addField(client.la[ls].cmds.info.serverinfo.field12, "<:booster:981883578109558854> \`" + message.guild.premiumSubscriptionCount + "\`", true)

      embed.addField(client.la[ls].cmds.info.serverinfo.field13, `<:booster:981883578109558854> \`${boostlevel}\``, true)
      embed.addField(client.la[ls].cmds.info.serverinfo.field14, "👾 \`" + maxbitrate + " kbps\`", true)
      if(boosts >= 14){
          embed.addField(`**<:arrow:832598861813776394> Vanity:**`, `${message.guild.vanityURLCode ? `https://discord.gg/${message.guild.vanityURLCode}` : "<:icons_Wrong:988298434354249809> No Vanity-Invite"}`)
      }
