    ...
    try {
      ...
      async function first_layer(){
        ...
        let MenuEmbed = new MessageEmbed()
          .setColor(es.color)
          .setAuthor('Welcome Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/samsung/306/waving-hand_1f44b?.png', 'https://discord.gg/stromz')
        ...
        collector.on('end', collected => {
          menumsg.edit({embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().values ? `<:icons_Correct:987931568939102228> **Selected: \`${collected ? collected.first().values[0] : "Nothing"}\`**` : "<:icons_Wrong:988298434354249809> **NOTHING SELECTED - CANCELLED**" }`})
        });
      }
      ...
      async function handle_the_picks(optionhandletype, SetupNumber, menuoptiondata) {
        switch (optionhandletype) {          
            ...
            async function second_layer(){
              let menuoptions = [
                ...
                },
                {
                  ...
                  emoji: "<:icons_Wrong:988298434354249809>"
                },
                {
                  ...
                  async function handle_the_picks_3(optionhandletype, SetupNumber, menuoptiondata){
                    switch (optionhandletype) {
                      ...
                      case `Frame Color`:{
                        ...
                        //Once the Collections ended edit the menu message
                        collector.on('end', collected => {
                          tempmsg.edit({embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `<:icons_Correct:987931568939102228> **Selected the \`${collected.first().customId}\` Color**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
                        });
                        //Menu Collections
          ...
          case `Test Welcome`:{
            ...
            let invitemessage = `Invited by ${member.user.tag ? `**${member.user.tag}**` : `<@${member.user.id}>`}\n<a:Like:988664548577910854> **X Invites**\n[<:joines:988347967364792320> X Joins | <:leave:988663133000306728> X Leaves | <:icons_Wrong:988298434354249809> X Fakes]`
            if(welcome){
