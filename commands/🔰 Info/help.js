    ...
    try {
      if (args[0]) {
        ...
        if (!cmd && (!cat || cat == null)) {
          ...
          if (cat == "🚫 Administration") return message.reply({ embeds: [embeds[5]] });
          if (cat == "💪 Setup") return message.reply({ embeds: [embeds[6]] });
          if (cat == "⚙️ Settings") return message.reply({ embeds: [embeds[7]] });
          if (cat == "👑 Owner") return message.reply({ embeds: [embeds[8]] });
          if (cat == "📈 Ranking") return message.reply({ embeds: [embeds[9]] });
          if (cat == "🎤 Voice") return message.reply({ embeds: [embeds[10]] });
          if (cat == "🕹️ Fun") return message.reply({ embeds: [embeds[11]] });
          if (cat == "🎮 MiniGames") return message.reply({ embeds: [embeds[12]] });
          if (cat == "🤞 Mincraft") return message.reply({ embeds: [embeds[13]] });
        ...
        let button_tutorial = new MessageButton().setStyle("LINK").setEmoji("840260133686870036").setLabel("Youtube").setURL("https://www.youtube.com/c/RaffiDev");
        let menuOptions = [
          {
            label: "Overview",
            value: "Overview",
        ...
        let menuSelection = new MessageSelectMenu().setCustomId("MenuSelection").setPlaceholder("Click me to view the Help Menu Category Pages").setMinValues(1).setMaxValues(5).addOptions(menuOptions.filter(Boolean));
...
> You can also mention ${client.user} to get prefix info.`
          ...
          .addField(
            "<a:a_b_badge:987910606923374594> **Categories**",
...
<:Fn_Minecraft:939104276540706826> Minecraft`,
        ...
        collector.on("collect", async (b) => {
          try {
            if (b?.isButton()) {
              if (b?.user.id !== message.author.id) return b?.reply({ content: handlemsg(client.la[ls].cmds.info.help.buttonerror, { prefix: prefix }), ephemeral: true });
      ...
      function allotherembeds_eachcategory(filterdisabled = false) {
        ...
        var embed0 = new MessageEmbed()
          ...
          .addField(
            `<:icons_exclamation:988292332203954247> **User Commands**`,
            ">>> " +
              client.commands
          ...
          .addField(
            `<:icons_exclamation:988292332203954247> **Games Related Commands**`,
            ">>> " +
              client.commands
          ...
          .addField(
            `<:icons_exclamation:988292332203954247> **Server Related Commands**`,
            ">>> " +
              client.commands
          ...
          .addField(
            `<:icons_exclamation:988292332203954247> **Bot Related Commands**`,
            ">>> " +
              client.commands
          ...
          .addField(
            `<:icons_exclamation:988292332203954247> **Util Related Commands**`,
            ">>> " +
              client.commands
        ...
        var embed1 = new MessageEmbed()
          ...
          .addField(
            `<:Builder:866089513654419466> **Information & Manage 💸**`,
            ">>> " +
              client.commands
        ...
        var embed3 = new MessageEmbed()
          .setTitle(
            `[\`${client.commands.filter((cmd) => cmd.category === "<:icons_music:990546169287544842> Music").size}\`] <:icons_music:988293666693054474> Music Commands <:icons_music:990546169287544842> | ${
              settings.MUSIC ? "<:icons_Correct:987931568939102228> ENABLED" : "<:icons_Wrong:988298434354249809> DISABLED"
            }`
          )
          .setDescription(
            "> ``addprevious``︲``addsimilar``︲``autoplay``︲``clearqueue``︲``forward``︲``grab``︲``join``︲``jump``︲``loop``︲``loopqueue``︲``loopsong``︲``lyrics``︲``move``︲``moveme``︲``nowplaying``︲``pause``︲``play``︲``playlist``︲``playmusicmix``︲``playprevious``︲``playsc``︲``playskip``︲``playskipsc``︲``playsongoftheday``︲``queue``︲``queuestatus``︲``radio``︲``reconnect``︲``removedupes``︲``removetrack``︲``restart``︲``resume``︲``rewind``︲``search``︲``searchplaylist``︲``searchradio``︲``searchsc``︲``searchsimilar``︲``seek``︲``shuffle``︲``skip``︲``stop``︲``stoploop``︲``unshuffle``︲``volume``"
          )
          .addField("\u200b", "__**Sub-Categorized Commands:**__")
          .addField(
            "<:icons_music:988293666693054474> **Queue Commands**",
            "> ``addprevious``︲``addsimilar``︲``autoplay``︲``clearqueue``︲``forward``︲``grab``︲``join``︲``jump``︲``loop``︲``loopqueue``︲``loopsong``︲``lyrics``︲``move``︲``moveme``︲``nowplaying``︲``pause``︲``play``︲``playlist``︲``playmusicmix``︲``playprevious``︲``playsc``︲``playskip``︲``playskipsc``︲``playsongoftheday``︲``queue``︲``queuestatus``︲``radio``︲``reconnect``︲``removedupes``︲``removetrack``︲``restart``︲``resume``︲``rewind``︲``search``︲``searchplaylist``︲``searchradio``︲``searchsc``︲``searchsimilar``︲``seek``︲``shuffle``︲``skip``︲``stop``︲``stoploop``︲``unshuffle``︲``volume``" +
              client.commands
                .filter((cmd) => cmd.category === "<:icons_music:988293666693054474> Music" && cmd.type.includes("queue"))
                .sort((a, b) => a.name.localeCompare(b?.name))
                .map((cmd) => `\`${cmd.name}\``)
                .join("︲")
          )
          .addField(
            "<a:Playing_Audio:859459305152708630> **Song Commands**",
            "> ``forward``︲``grab``︲``loop``︲``loopsong``︲``lyrics``︲``nowplaying``︲``pause``︲``play``︲``playlist``︲``playmusicmix``︲``playprevious``︲``playsc``︲``playskip``︲``playskipsc``︲``playsongoftheday``︲``radio``︲``restart``︲``resume``︲``rewind``︲``seek``︲``skip``︲``stop``︲``stoploop``︲``volume``" +
              client.commands
                .filter((cmd) => cmd.category === "<:icons_music:988293666693054474> Music" && cmd.type.includes("song"))
                .sort((a, b) => a.name.localeCompare(b?.name))
                .map((cmd) => `\`${cmd.name}\``)
                .join("︲")
          )
          .addField(
            "<:Bot_Flag:835928340715012137> **Bot Commands**",
            "> ``join``︲``moveme``︲``reconnect``" +
              client.commands
                .filter((cmd) => cmd.category === "<:icons_music:988293666693054474> Music" && cmd.type.includes("bot"))
                .sort((a, b) => a.name.localeCompare(b?.name))
                .map((cmd) => `\`${cmd.name}\``)
                .join("︲")
          );
        ...
        var embed5 = new MessageEmbed()
          ...
          .addField("\u200b", "__**Sub-Categorized Commands:**__")
          .addField(
            "<:6149_discord:971276208027340810> **Server Related Commands**",
            "> " +
              client.commands
          ...
          .addField(
            "<:staff:988343906905235476> **Channel Related Commands**",
            "> " +
              client.commands
          ...
          .addField(
            "<:staff:988343906905235476> **Thread Related Commands**",
            "> " +
              client.commands
          ...
          .addField(
            "<:staff:988343906905235476> **Role Related Commands**",
            "> " +
              client.commands
          ...
          .addField(
            "<:staff:988343906905235476> **Member Related Commands**",
            "> " +
              client.commands
        ...
        var embed6 = new MessageEmbed()
          ...
          .addField("\u200b", "__**Sub-Categorized Commands:**__")
          .addField(
            "<:icons_settings:988295321262108692> **Setups for Entertainment**",
            "> " +
              client.commands
          ...
          .addField(
            ...
            "> ``setup``︲``setup-admin``︲``setup-admincmdlog``︲``setup-commands``︲``setup-embed``︲``setup-epicgamesverify``︲``setup-language``︲``setup-leave``︲``setup-welcome``" +
              client.commands
                .filter((cmd) => cmd.category === "<:icons_settings:988295321262108692> Setup" && cmd.type.includes("info"))
                .sort((a, b) => a.name.localeCompare(b?.name))
                .map((cmd) => `\`${cmd.name}\``)
                .join("︲")
          )
          .addField(
            "<:blurple_shield:958372860487929856> **Most used Systems**",
            "> ``setup-apply``︲``setup-autodelete``︲``setup-autoembed``︲``setup-autosupport``︲``setup-boost``︲``setup-boostlog``︲``setup-customcommand``︲``setup-joinvc``︲``setup-jtc``︲``setup-keyword``︲``setup-membercount``︲``setup-menuapply``︲``setup-menuticket``︲``setup-rank``︲``setup-reactionrole``︲``setup-roster``︲``setup-serverstats``︲``setup-suggestion``︲``setup-ticket``" +
              client.commands
                .filter((cmd) => cmd.category === "<:icons_settings:988295321262108692> Setup" && cmd.type.includes("system"))
                .sort((a, b) => a.name.localeCompare(b?.name))
                .map((cmd) => `\`${cmd.name}\``)
                .join("︲")
          )
          .addField(
            "<:Builder:866089513654419466> **Security Systems**",
            "> ``setup-anticaps``︲``setup-antidiscord``︲``setup-antilink``︲``setup-antimassmention``︲``setup-antimasspings``︲``setup-antimention``︲``setup-antinewaccount``︲``setup-antinuke``︲``setup-antipings``︲``setup-antiscamlinks``︲``setup-antiselfbot``︲``setup-antispam``︲``setup-auditlog``︲``setup-autobackup``︲``setup-autowarn``︲``setup-blacklist``︲``setup-ghost-ping-detector``︲``setup-joinlist``︲``setup-logger``︲``setup-reportlog``︲``setup-warn``" +
              client.commands
                .filter((cmd) => cmd.category === "<:icons_settings:988295321262108692> Setup" && cmd.type.includes("security"))
                .sort((a, b) => a.name.localeCompare(b?.name))
                .map((cmd) => `\`${cmd.name}\``)
                .join("︲")
          );
        ...
        var embed7 = new MessageEmbed()
          ...
          .addField("\u200b", "__**Sub-Categorized Commands:**__")
          .addField(
            "<:icons_settings:988295321262108692> **User Related Commands**",
            "> " +
              client.commands
          ...
          .addField(
            "<:icons_settings:988295321262108692> **Bot Related Commands**",
            "> " +
              client.commands
          ...
          .addField(
            "<:icons_music:990546169287544842> **Music Related Commands**",
            "> " +
              client.commands
        ...
        var embed8 = new MessageEmbed()
          ...
          .addField("\u200b", "__**Sub-Categorized Commands:**__")
          .addField(
            "<:Discord:787321652345438228> **Information & Manage**",
            "> " +
              client.commands
          ...
          .addField(
            "<:Bot_Flag:835928340715012137> **Adjust the Bot**",
            "> " +
              client.commands
        ...
        var embed9 = new MessageEmbed()
          ...
          .addField("\u200b", "__**Sub-Categorized Commands:**__")
          .addField(
            "<:Builder:866089513654419466> **Manage Rank**",
            `> ${client.commands
        ...
        var embed10 = new MessageEmbed()
          .setTitle(
            `[\`${client.commands.filter((cmd) => cmd.category === "🎤 Voice").first().extracustomdesc.length}\`] 🎤 Voice Commands 🎤 | ${
              settings.VOICE ? "<:icons_Correct:987931568939102228> ENABLED" : "<:icons_Wrong:988298434354249809> DISABLED"
            }`
          )
          .setDescription(
            `> *${client.commands
        ...
        var embed11 = new MessageEmbed()
          ...
          .addField("\u200b", "__**Sub-Categorized Commands:**__")
          .addField(
            "<:icons_games:988293479392223242> **Fun User Image Commands**",
            "> " +
              client.commands
          ...
          .addField(
            "<:icons_games:988293479392223242> **Fun User Image-Text Commands**",
            "> " +
              client.commands
          ...
          .addField(
            "<:icons_games:988293479392223242> **Fun Text Commands**",
            "> " +
              client.commands
        ...
        var embed12 = new MessageEmbed()
          .setTitle(
            `[\`${client.commands.filter((cmd) => cmd.category === "🎮 MiniGames").size}\`] <:icons_games:988293479392223242> Mini Games Commands <:icons_games:988293479392223242> | ${
              settings.MINIGAMES ? "<:icons_Correct:987931568939102228> ENABLED" : "<:icons_Wrong:988298434354249809> DISABLED"
            }`
          )
          .addField("\u200b", "__**Sub-Categorized Commands:**__")
          .addField(
            "<:icons_games:988293479392223242> **Text Based Minigames**",
            "> " +
              client.commands
          ...
          .addField(
            "<:icons_games:988293479392223242> **Button(s) Minigames**",
            "> " +
              client.commands
          ...
          .addField(
            "<:icons_games:988293479392223242> **Voice Minigames**",
            "> " +
              client.commands
        ...
        var embed13 = new MessageEmbed()
          .setTitle(
            `[\`${client.commands.filter((cmd) => cmd.category === "🤞 Mincraft").size}\`] <:Fn_Minecraft:939104276540706826> Minecraft Commands <:Fn_Minecraft:939104276540706826> | ${
              settings.MINECRAFT ? "<:icons_Correct:987931568939102228> ENABLED" : "<:icons_Wrong:988298434354249809> DISABLED"
            }`
          )
          .addField(
            "<:Fn_Minecraft:939104276540706826> **Minecrafts**",
            "> ``server``︲``skin``︲``uuid``" +
              client.commands
                .filter((cmd) => cmd.category === "🤞 Mincraft" && cmd.type === "voice")
                .sort((a, b) => a.name.localeCompare(b?.name))
                .map((cmd) => `\`${cmd.name}\``)
                .join("︲")
          )
          .setDescription(
            `> *${client.commands
              .filter((cmd) => cmd.category === "🤞 Mincraft")
              .sort((a, b) => a.name.localeCompare(b?.name))
              .map((cmd) => `\`${cmd.name}\``)
              .join("︲")}*`
          );
      ...
      return message.reply({ embeds: [new MessageEmbed().setColor(es.wrongcolor).setFooter(client.getFooter(es)).setTitle(client.la[ls].common.erroroccur).setDescription(eval(client.la[ls]["cmds"]["info"]["color"]["variable2"]))] });
    }
  },
};
/**
