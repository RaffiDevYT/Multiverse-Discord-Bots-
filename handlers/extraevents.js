...
module.exports = client => {
  ...
  client.getFooter = (es, stringurl = null) => {
    ...
    if(!text || text.length < 1) text = `${client.user.username} | By: RaffiDev#8321`;
  ...
  client.getAuthor = (authorname = null, authoricon = null, authorurl = null) => {
    ...
    if(!name || name.length < 1) name = `${client.user.username} | By: RaffiDev#8321`;
    if(!iconURL || iconURL.length < 1) iconURL = `${client.user.displayAvatarURL()}`;
    if(!url || url.length < 1) url = `https://discord.gg/DcR5n6PTFf`;
    ...
    //verify the iconURL
    if(!url.startsWith("https://") && !url.startsWith("http://")) url = `https://discord.gg/DcR5n6PTFf`;
    if(!iconURL.startsWith("https://") && !iconURL.startsWith("http://")) iconURL = client.user.displayAvatarURL();
    if(![".png", ".jpg", ".wpeg", ".webm", ".gif"].some(d => iconURL.toLowerCase().endsWith(d))) iconURL = client.user.displayAvatarURL();
    //return the footerobject
  ...
  */
  ...
  client.on("guildCreate", async guild => {
    ...
    let embed = new MessageEmbed()
      .setColor("GREEN")
      .setTitle(`<:joines:988347967364792320> Joined a New Server`)
    ...
    for(const owner of config.ownerIDS){
      //If the Owner is RaffiDev, and the Bot is in not a Stromz Development, Public Bot, then dont send information!
      if(owner == "847710668005572610"){
        let stromzGuild = client.guilds.cache.get("966232465901510656");
        if(stromzGuild && !stromzGuild.me.roles.cache.has("966232465901510656")){
          continue; 
        }
      }
