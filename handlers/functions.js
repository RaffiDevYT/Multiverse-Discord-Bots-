      ...
      if (data.rosterstyle == "1") {
        ...
        for (let i = 0; i < memberarray.length; i += 20) {
          ...
          if (!the_roster_db?.get(guild.id, "showallroles") || memberarray.length < 20)
            try { 
              rosterembed.addField(`**__${role.name.toUpperCase()} [${role.members.map(this_Code_is_by_RaffiDev_8321 => this_Code_is_by_RaffiDev_8321).length}]__**`, role.members.map(this_Code_is_by_RaffiDev_8321 => this_Code_is_by_RaffiDev_8321).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substr(0, leftnum <= 1024 ? leftnum : 1024) + `${role.members.map(this_Code_is_by_RaffiDev_8321 => this_Code_is_by_RaffiDev_8321).length - 20 > 0 ? `\n${the_roster_db?.get(guild.id, "rosteremoji")} ***\`${role.members.map(this_Code_is_by_RaffiDev_8321 => this_Code_is_by_RaffiDev_8321).length - 20}\` other Members have this Role ...***`: ""}`.substr(0, 1024), the_roster_db?.get(guild.id, "inline"))
              break;
            } catch (e) {
            ...
            try {
              rosterembed.addField(i < 20 ? `**__${role.name.toUpperCase()} [${role.members.map(this_Code_is_by_RaffiDev_8321 => this_Code_is_by_RaffiDev_8321).length}]__**` : `\u200b`, role.members.map(this_Code_is_by_RaffiDev_8321 => this_Code_is_by_RaffiDev_8321).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substr(0, leftnum <= 1024 ? leftnum : 1024), the_roster_db?.get(guild.id, "inline"))
            } catch (e) {
              console.log(e.stack ? String(e.stack).grey : String(e).grey)
            }
        }
